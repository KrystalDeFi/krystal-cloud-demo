...
const VirtualizedTableRow = React.memo(({ 
  ...
  return (
    ...
    >
      ...
      <Td>
        <Text fontWeight="medium">{Formatter.formatCurrency(position.currentPositionValue || 0)}</Text>
      </Td>
      <Td>
        <Text 
      ...
      <Td>
        <Text fontWeight="medium">{Formatter.formatCurrency(getTotalPendingFees(position))}</Text>
      </Td>
      <Td>
...
export default function WalletPositionsPage() {
  ...
  const memoizedFilteredPositions = useMemo(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(position =>
        position.pool.protocol.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        position.pool.poolAddress.toLowerCase().includes(searchTerm.toLowerCase()) ||
        position.chain.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        position.currentAmounts.some(amount => 
          amount.token.symbol.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    if (selectedChain !== "all") {
      filtered = filtered.filter(position => position.chain.id.toString() === selectedChain);
    }
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case "apr":
          ...
          break;
        case "fees":
          aValue = a.tradingFee.pending.reduce((sum, fee) => sum + fee.value, 0);
          bValue = b.tradingFee.pending.reduce((sum, fee) => sum + fee.value, 0);
          break;
        case "age":
    ...
    return filtered;
  }, [openPositions, closedPositions, selectedStatus, searchTerm, selectedChain, sortField, sortOrder]);
  ...
  const memoizedStats = useMemo(() => {
    ...
    const totalValue = allPositions.reduce((sum, pos) => sum + (pos.currentPositionValue || 0), 0);
    const activePositions = openPositions.length;
    const totalFeesEarned = allPositions.reduce((sum, pos) => {
      const pendingFees = pos.tradingFee.pending.reduce((feeSum, fee) => feeSum + fee.value, 0);
      const claimedFees = pos.tradingFee.claimed.reduce((feeSum, fee) => feeSum + fee.value, 0);
      return sum + pendingFees + claimedFees;
    }, 0);
    const averageApr = allPositions.length > 0 ? 
      allPositions.reduce((sum, pos) => sum + pos.performance.apr.totalApr, 0) / allPositions.length : 0;
  ...
  const handleWalletAddressKeyPress = useCallback((e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      handleWalletAddressChange();
    }
  }, [handleWalletAddressChange]);

  const handleRowClick = useCallback((position: IAPosition) => {
  ...
  return (
    <Box minH="100vh" bg="gray.50" _dark={{ bg: "gray.900" }}>
      <Container maxW="7xl" py={6}>
          ...
          <Card bg={cardBg} p={6} mb={6} border="1px" borderColor={borderColor}>
            <HStack spacing={8} justify="space-around">
              <VStack>
                ...
                <Text fontSize="2xl" fontWeight="bold">{Formatter.formatCurrency(memoizedStats.totalValue)}</Text>
              </VStack>
              <VStack>
                <Text fontSize="sm" color="gray.500">Active Positions</Text>
                <Text fontSize="2xl" fontWeight="bold">{memoizedStats.activePositions}</Text>
              </VStack>
              <VStack>
                <Text fontSize="sm" color="gray.500">Total Fees Earned</Text>
                <Text fontSize="2xl" fontWeight="bold">{Formatter.formatCurrency(memoizedStats.totalFeesEarned)}</Text>
              </VStack>
              <VStack>
                <Text fontSize="sm" color="gray.500">Average APR</Text>
                <Text fontSize="2xl" fontWeight="bold" color="green.500">{Formatter.formatAPR(memoizedStats.averageApr)}</Text>
              </VStack>
            </HStack>
          </Card>
        )}
        ...
        {/* Tabs */}
        <Tabs defaultIndex={0} onChange={(index) => setSelectedStatus(index === 0 ? "OPEN" : "CLOSED")}>
          <TabList>
          ...
          <TabPanels>
            <TabPanel p={0} pt={6}>
              ...
              <HStack spacing={4} mb={6} wrap="wrap">
                ...
                >
                  ...
                  {Array.from(new Set(openPositions.map(p => p.chain.id))).map(chainId => (
                    <option key={chainId} value={chainId}>
                      {openPositions.find(p => p.chain.id === chainId)?.chain.name}
                    </option>
                  ))}
                </Select>
              </HStack>
                ...
                <TableContainer>
                  <Table variant="simple">
                    <Thead>
                      <Tr>
                        <Th>Pool</Th>
                        <Th cursor="pointer" onClick={() => handleSort("currentPositionValue")}>
                          <HStack spacing={1}>
              ...
              {hasMore && (
                <Box textAlign="center" mt={4}>
                  <Button
                    ...
                    size="lg"
                  >
                    Load More ({memoizedFilteredPositions.length - paginatedPositions.length} remaining)
                  </Button>
                </Box>
              )}
              ...
              <Box textAlign="center" mt={4}>
                <Text fontSize="sm" color="gray.500">
                  Showing {paginatedPositions.length} of {memoizedFilteredPositions.length} positions
                </Text>
              </Box>
            </TabPanel>
            ...
            <TabPanel p={0} pt={6}>
              ...
              <HStack spacing={4} mb={6} wrap="wrap">
                ...
                >
                  <option value="all">All Chains</option>
                  {Array.from(new Set(closedPositions.map(p => p.chain.id))).map(chainId => (
                    <option key={chainId} value={chainId}>
                      {closedPositions.find(p => p.chain.id === chainId)?.chain.name}
                    </option>
                  ))}
                </Select>
              </HStack>
