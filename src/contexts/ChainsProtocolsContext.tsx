...
export const ChainsProtocolsProvider: React.FC<ChainsProtocolsProviderProps> = ({ children }) => {
  const [chains, setChains] = useState<IAChain[]>(CacheService.get(CHAINS_CACHE_KEY) || DEFAULT_CHAINS);
  const [protocols, setProtocols] = useState<IAProtocol[]>(CacheService.get(PROTOCOLS_CACHE_KEY) || Object.values(DEFAULT_PROTOCOLS));
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : 'Failed to load chains and protocols');
      } finally {
        setLoading(false);
      }
    };
